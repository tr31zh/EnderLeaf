...
class Stage(SerialDevice):
    ...
    async def write_code(self, code, check_ok=True, debug=False, call_back=None):
    ...
    async def set_speed(self, speed, debug=False, call_back=None):
        """
        ...
        """
        code = f"G0 F{speed}"
        await self.write_code(code, debug=debug, call_back=call_back)

    async def set_speed_limit(self, speed, axis="x", debug=False, call_back=None):
        """
        Sets the speed of the stage
        ...
        await self.write_code(
            f'{G_CODES["set_speed_limit"]} {axis.upper()}{speed}',
            debug=debug,
            call_back=call_back,
        )

    async def move_absolute(self, x, y, z=None, debug=False, call_back=None):
        """
    ...
    async def move_position(self, p, debug=False, call_back=None):
        """
    ...
    async def move_relative(self, x, y, z=None, debug=False, call_back=None):
        """
        ...
        :return:
        """
        await self.set_relative(debug=debug)
        if z is None:
            code = f"G0 X {x} Y {y}"
        else:
            code = f"G0 X {x} Y {y} Z {z}"
    ...
    async def move_towards(self, direction, distance, debug=False, call_back=None):
        """
        Moves the stage in the given direction
        :param direction:
        :return:
        """
        await self.set_relative()
    ...
    async def move_axis(self, axis, distance, debug=False, call_back=None):
        """
    ...
    async def get_position(self, dict=False, debug=False, call_back=None):
        self.flush_serial_buffer()
        response = await self.write_code(
            G_CODES["current_position"], check_ok=False, call_back=call_back
        )
        if debug:
            print(response)
    ...
    async def home(self, debug=False, call_back=None):
        await self.write_code(G_CODES["homing"], debug=debug, call_back=call_back)

    async def safe_home(self, debug=False, call_back=None):
        before = timer()
        await self.home(debug=debug, call_back=call_back)
        after = timer()
        return after - before > 2

    async def finish_moves(self, debug=False, call_back=None):
        await self.write_code(G_CODES["finish"], debug=debug, call_back=call_back)

    async def set_relative(self, debug=False, call_back=None):
        await self.write_code(G_CODES["relative"], debug=debug, call_back=call_back)

    async def set_absolute(self, debug=False, call_back=None):
        await self.write_code(G_CODES["absolute"], debug=debug, call_back=call_back)
